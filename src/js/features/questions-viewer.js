...
function renderMcqQuestion(q, index) {
    ...
    const expHtml = q.explanation ? `<div id="${expId}" class="question-explanation" style="display:none;"><strong>💡 توضيح:</strong> ${esc(q.explanation)}</div>` : '';
...
function renderShortQuestion(q, index) {
    ...
    const expHtml = q.explanation ? `<div class="question-explanation" style="margin-top:10px;"><strong>💡 توضيح:</strong> ${esc(q.explanation)}</div>` : '';
